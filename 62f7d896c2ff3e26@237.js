...
htl.html`<h1> Observing CO2 Emissions by Sectors Globally </h1>
<h4> from 1990 - 2019 </h4>`
)}

function _2(htl){return(
htl.html`<p> To run the Running Bar Chart, simply click the "Play" button. You can toggle with the Scrubber too :-)! </p>`
)}
...
d3.csvParse(await FileAttachment("2. (new) GHG Emissions by Sector Dataset (1990-2019).csv").text(), d3.autoType)
)}
...
Array.from(d3.rollup(data, ([d]) => d.emissions, d => +d.date, d => d.sector))
...
function rank(value) {
  const data = Array.from(names, sector => ({sector, value: value(sector)}));
...
function _color(d3,data)
{
  ...
  if (data.some(d => d.sector !== undefined)) {
    const SectorByName = new Map(data.map(d => [d.sector, d.sector]))
    scale.domain(Array.from(SectorByName.values()));
    return d => scale(SectorByName.get(d.sector));
  }
  return d => scale(d.sector);
}


function _axis(margin,d3,x,barSize,n,y){return(
function axis(svg) {
  ...
  for ([[ka, a], [kb, b]] of d3.pairs(Yearvalues)) {
    for (let i = 0; i < k; ++i) {
      ...
      keyframes.push([
        ...
        rank(sector => (a.get(sector) || 0) * (1 - t) + (b.get(sector) || 0) * t)
      ]);
    }
  }
  keyframes.push([new Date(kb), rank(sector => b.get(sector) || 0)]);
  return keyframes;
}


function _sectorframes(d3,keyframes){return(
d3.groups(keyframes.flatMap(([, data]) => data), d => d.sector)
)}

function _prev(sectorframes,d3){return(
new Map(sectorframes.flatMap(([, data]) => d3.pairs(data, (a, b) => [b, a])))
)}

function _next(sectorframes,d3){return(
new Map(sectorframes.flatMap(([, data]) => d3.pairs(data)))
)}
...
function bars(svg) {
  ...
  return ([date, data], transition) => bar = bar
    .data(data.slice(0, n), d => d.sector)
    .join(
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    ["2. (new) GHG Emissions by Sector Dataset (1990-2019).csv", {url: new URL("./files/c255512902d9b232ec620cfcf9bdc179d87cf5d9ad2515d3eaf1976982dda9801109c1139e5b9f0ec9b2c18850823ee65e7adcdbcab342f415d7235672f90232.csv", import.meta.url), mimeType: "text/csv", toString}]
  ]);
  ...
  main.variable(observer("axis")).define("axis", ["margin","d3","x","barSize","n","y"], _axis);
  main.variable(observer("keyframes")).define("keyframes", ["d3","Yearvalues","k","rank"], _keyframes);
  main.variable(observer("sectorframes")).define("sectorframes", ["d3","keyframes"], _sectorframes);
  main.variable(observer("prev")).define("prev", ["sectorframes","d3"], _prev);
  main.variable(observer("next")).define("next", ["sectorframes","d3"], _next);
